    ...
    try {
        ...
        if (req.body.type === "All") {
            ...
            const users = await User.find(query)
                .select('full_name diet_plan_status workout_plan_status subsctiption_status').populate({ path: 'coach_id', select: '_id full_name email role U_ID' }).populate({ path: 'workoutCoach_id', select: '_id full_name email role U_ID' })
                .limit(pageSize)
                .skip(skip)
                .exec();
